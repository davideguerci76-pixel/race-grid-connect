        ...
        <>
          ...
          <div className={`absolute top-full z-50 mt-1 min-w-[100px] border border-border bg-card shadow-lg ${align === "left" ? "left-0" : "right-0"}`}>
            {SUPPORTED_LANGS.map((l) => (
              <button
                key={l.code}
                onClick={() => { void i18n.changeLanguage(l.code); try { window.localStorage.setItem(LANG_STORAGE_KEY, l.code); document.documentElement.lang = l.code; } catch { /* ignore */ } setOpen(false); }}
