...
export function PitCallRevealTeaser({ detailCount, cost = 1, hideCost = false }: { detailCount?: number; cost?: number; hideCost?: boolean }) {
  ...
  return (
    <div className="mt-3 border border-dashed border-border bg-background/40 p-3">
      ...
      <p className="text-xs text-muted-foreground">{hideCost ? t("reveal.locked_body_plain") : cost > 0 ? t("reveal.locked_body", { count: cost }) : t("reveal.locked_body_free")}</p>
