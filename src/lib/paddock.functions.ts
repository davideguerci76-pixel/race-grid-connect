...
export const getMyEngagements = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabase
      .from("engagements")
      .select("*, request:requests(id, title, role_group, sub_role, sub_role_min_level, discipline, start_date, end_date, season_dates, skills, skills_hard, education, languages, budget_min, budget_max, budget_unit, notes, location, circuit, duration, team_grace_reopen_used_at), match:matches(id, match_score, is_perfect, overlap_days, missing_criteria, revealed_by_freelancer, revealed_by_team)")
