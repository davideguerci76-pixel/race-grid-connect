...
ALTER TABLE public.freelancer_profiles
  ADD COLUMN IF NOT EXISTS calendar_last_updated_at timestamptz NOT NULL DEFAULT now();

-- 2) Add weight column and rebalance defaults
ALTER TABLE public.matching_weights
  ADD COLUMN IF NOT EXISTS calendar_freshness_weight numeric NOT NULL DEFAULT 4;

ALTER TABLE public.matching_weights ALTER COLUMN role_weight SET DEFAULT 35;
ALTER TABLE public.matching_weights ALTER COLUMN skills_weight SET DEFAULT 24;

UPDATE public.matching_weights
  SET role_weight = 35,
      skills_weight = 24,
      calendar_freshness_weight = 4,
      updated_at = now()
  WHERE id = true;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'calendar_stale'
    AND enumtypid = (SELECT oid FROM pg_type WHERE typname = 'notif_kind'))
  THEN
    ALTER TYPE public.notif_kind ADD VALUE 'calendar_stale';
  END IF;
END $$;

-- 4) RPC to confirm calendar (one-click)
...
BEGIN
  ...
  UPDATE public.freelancer_profiles
    SET calendar_last_updated_at = _ts, updated_at = _ts
    WHERE user_id = _uid;
  IF NOT FOUND THEN
...
-- 5) Bump calendar timestamp when availability changes
CREATE OR REPLACE FUNCTION public.tg_bump_calendar_freshness()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  _uid uuid := COALESCE(NEW.freelancer_id, OLD.freelancer_id);
BEGIN
  UPDATE public.freelancer_profiles
    SET calendar_last_updated_at = now()
    WHERE user_id = _uid;
  RETURN COALESCE(NEW, OLD);
END; $$;

DROP TRIGGER IF EXISTS availability_bump_freshness ON public.availability;
CREATE TRIGGER availability_bump_freshness
AFTER INSERT OR DELETE ON public.availability
FOR EACH ROW EXECUTE FUNCTION public.tg_bump_calendar_freshness();

-- 6) Stale reminder emitter (idempotent within 30 days)
...
BEGIN
  WITH candidates AS (
    ...
    WHERE fp.calendar_last_updated_at < (now() - interval '30 days')
      AND NOT EXISTS (
...
-- 7) Recompute matches with freshness score
...
BEGIN
  ...
  computed AS (
    SELECT p.*,
      LEAST(100, ROUND(role_s + skills_s + disc_s + rate_s + langs_s + edu_s + loc_s + fresh_s, 2)) AS skills_score_v,
...
-- Kick a full recompute so freshness scoring applies immediately
