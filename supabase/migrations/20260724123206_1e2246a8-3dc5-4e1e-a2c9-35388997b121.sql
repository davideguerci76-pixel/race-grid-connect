...
ALTER TABLE public.requests
  ADD COLUMN IF NOT EXISTS travel_required boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS skills_hard text[] NOT NULL DEFAULT '{}';

-- Update create_request to accept the new fields
...
BEGIN
  ...
  IF _payload ? 'skills_hard' AND jsonb_typeof(_payload->'skills_hard') = 'array' THEN
    SELECT ARRAY(SELECT (value #>> '{}')::text FROM jsonb_array_elements(_payload->'skills_hard'))
    INTO _skills_hard;
  END IF;
  ...
  INSERT INTO public.requests(
    ...
    budget_min, budget_max, budget_unit, notes, season_dates, skills, skills_hard,
...
-- Update recompute_matches: skills becomes soft (no filter), skills_hard hard-filters, travel_required hard-filters against fp.travels
...
BEGIN
  WITH valid_matches AS (
    ...
    JOIN public.freelancer_profiles fp
     ...
     AND (
       COALESCE(r.travel_required, true) = false
       OR COALESCE(fp.travels, false) = true
     )
     AND (
       COALESCE(array_length(r.skills_hard, 1), 0) = 0
       OR fp.skills @> r.skills_hard
     )
     AND (
