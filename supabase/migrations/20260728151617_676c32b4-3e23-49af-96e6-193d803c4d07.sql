...
BEGIN
  ...
  -- Overlap guard: no other active/locked engagement for this freelancer may overlap these dates
  SELECT id INTO _conflict_id
  FROM public.engagements
  WHERE freelancer_id = _e.freelancer_id
    AND id <> _e.id
    AND start_date <= _e.end_date
    AND end_date   >= _e.start_date
    AND (
      status = 'confirmed'
      OR (status = 'cancelled' AND cancellation_kind IN ('freelancer_late','no_show'))
    )
  LIMIT 1;
