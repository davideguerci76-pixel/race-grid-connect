...
ALTER TABLE public.requests
  ADD COLUMN IF NOT EXISTS ever_full_matched boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS ever_partial_matched boolean NOT NULL DEFAULT false;
...
BEGIN
  UPDATE public.requests r
   ...
   WHERE r.id = NEW.request_id
     AND (r.ever_full_matched IS DISTINCT FROM (r.ever_full_matched OR (NEW.is_partial = false))
...
AFTER INSERT OR UPDATE OF is_partial ON public.matches
FOR EACH ROW EXECUTE FUNCTION public.tg_mark_request_ever_matched();

UPDATE public.requests r
   SET ever_full_matched = true
 WHERE EXISTS (SELECT 1 FROM public.matches m WHERE m.request_id = r.id AND m.is_partial = false);
UPDATE public.requests r
   SET ever_partial_matched = true
 WHERE EXISTS (SELECT 1 FROM public.matches m WHERE m.request_id = r.id AND m.is_partial = true);

UPDATE public.requests r
   SET ever_full_matched = true
 WHERE r.ever_full_matched = false
   AND EXISTS (SELECT 1 FROM public.match_history h WHERE h.request_id = r.id);
...
BEGIN
  ...
  SELECT EXISTS(SELECT 1 FROM public.matches WHERE request_id = _request_id AND is_partial = false)
         OR COALESCE(_r.ever_full_matched, false) INTO _has_full;
