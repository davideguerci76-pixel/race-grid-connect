-- STEP 9 correction (forward-only):
--  1) propagate the environment flag onto the emitted notification rows
--  2) keep the freelancer payload anonymous (no request_id / team hint)
--  3) scope the engagement guard to the same environment
-- hot_partial_state PK (request_id, freelancer_id) stays environment-safe because
-- a request row belongs to exactly one environment (requests.is_test).
