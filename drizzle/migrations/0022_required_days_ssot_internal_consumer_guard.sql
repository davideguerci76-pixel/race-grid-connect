-- STEP 4C safety correction: allow only the internal recompute consumer to read
-- required days across its scoped requests. Direct authenticated calls remain
-- Team/Freelancer scoped. No data rows are changed.
...
DECLARE
  _uid uuid := auth.uid();
  _internal boolean := COALESCE(current_setting('pitcall.internal_required_days', true), 'false') = 'true';
  _season_dates date[];
  _start_date date;
  _end_date date;
  _required_days date[];
...
BEGIN
  PERFORM set_config('pitcall.internal_required_days', 'true', true);
  ...
  final AS (
    SELECT freelancer_id, team_id, request_id, overlap_days, skills_score_v AS skills_score,
      ...
      ((CASE WHEN r_sub_role IS NOT NULL AND subrole_s < sub_role_weight THEN jsonb_build_array(jsonb_build_object('kind','sub_role','label',r_sub_role,'level',r_sub_level,'hard',r_sub_hard)) ELSE '[]'::jsonb END)
       ...
       || COALESCE((SELECT jsonb_agg(jsonb_build_object('kind','language','code',lreq->>'code','level',lreq->>'level','hard',COALESCE((lreq->>'hard')::boolean,false))) FROM jsonb_array_elements(r_langs) lreq WHERE NOT EXISTS (SELECT 1 FROM jsonb_array_elements(f_langs) flang WHERE lower(coalesce(flang->>'code','')) = lower(coalesce(lreq->>'code','')) AND (CASE lower(coalesce(flang->>'level','')) WHEN 'basic' THEN 1 WHEN 'intermediate' THEN 2 WHEN 'advanced' THEN 3 WHEN 'fluent' THEN 4 WHEN 'native' THEN 5 ELSE 1 END) >= (CASE lower(coalesce(lreq->>'level','')) WHEN 'basic' THEN 1 WHEN 'intermediate' THEN 2 WHEN 'advanced' THEN 3 WHEN 'fluent' THEN 4 WHEN 'native' THEN 5 ELSE 1 END))),'[]'::jsonb)
  ...
  INSERT INTO public.matches (freelancer_id, team_id, request_id, overlap_days, score, match_score, missing_criteria, is_perfect, missing_days, missing_pct, is_partial, edge_only, skills_score, final_score)
  SELECT freelancer_id, team_id, request_id, overlap_days, overlap_days::numeric, skills_score, missing_criteria, (skills_score >= 100 AND missing_days = 0), missing_days, missing_pct, is_partial, edge_only, skills_score, final_score
  FROM final
  ON CONFLICT (freelancer_id, request_id) DO UPDATE SET stale = false, overlap_days = EXCLUDED.overlap_days, score = EXCLUDED.score, match_score = EXCLUDED.match_score, missing_criteria = EXCLUDED.missing_criteria, is_perfect = EXCLUDED.is_perfect, missing_days = EXCLUDED.missing_days, missing_pct = EXCLUDED.missing_pct, is_partial = EXCLUDED.is_partial, edge_only = EXCLUDED.edge_only, skills_score = EXCLUDED.skills_score, final_score = EXCLUDED.final_score;
