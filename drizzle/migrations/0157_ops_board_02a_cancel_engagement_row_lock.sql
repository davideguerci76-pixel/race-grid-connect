-- OPS-BOARD-02A — make the cancellation itself atomic: lock the engagement row
-- before the status check so two concurrent cancels cannot both pass the guard
-- (single valid cancellation, single notification, single reopen).
