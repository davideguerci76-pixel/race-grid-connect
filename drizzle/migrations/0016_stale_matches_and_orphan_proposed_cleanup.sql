-- Current match state vs Request Confirmation snapshot.
-- Match rows kept alive only because an engagement references them are flagged
-- stale so they are never presented as a current match.
...
BEGIN
  -- Everything in scope is stale until this run re-validates it. Rows kept only
  -- because an engagement references them stay stale and are hidden from users.
  ...
  deleted AS (
    ...
    WHERE (_freelancer_id IS NULL OR m.freelancer_id = _freelancer_id)
      ...
      AND NOT EXISTS (SELECT 1 FROM final f WHERE f.freelancer_id = m.freelancer_id AND f.request_id = m.request_id) AND NOT EXISTS (SELECT 1 FROM public.engagements e WHERE e.match_id = m.id AND e.status IN ('proposed','confirmed','completed'))
    RETURNING 1
  )
...
-- Any real close/cancel path releases pending Request Confirmations (and their
-- Frozen Green days). Idempotent: close_proposed_for_request is a no-op when
-- nothing is proposed.
CREATE OR REPLACE FUNCTION public.tg_close_proposed_on_request_close()
RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
BEGIN
  PERFORM public.close_proposed_for_request(NEW.id, 'request_closed');
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS requests_close_proposed ON public.requests;
CREATE TRIGGER requests_close_proposed
  AFTER UPDATE OF status ON public.requests
  FOR EACH ROW
  WHEN (OLD.status IS DISTINCT FROM NEW.status AND NEW.status IN ('closed','completed'))
  EXECUTE FUNCTION public.tg_close_proposed_on_request_close();
