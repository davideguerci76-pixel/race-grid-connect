-- UAT-ONBOARD-05 — robustness: an empty candidate list must return zero rows, not raise (FOREACH over NULL).
...
BEGIN
  ...
  FOREACH _cid IN ARRAY COALESCE((SELECT array_agg(DISTINCT x) FROM unnest(COALESCE(_candidate_ids, ARRAY[]::uuid[])) AS x), ARRAY[]::uuid[]) LOOP
