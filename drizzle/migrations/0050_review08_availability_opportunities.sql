-- REVIEW08 — Availability Opportunities for NORMAL Pit Calls only.
-- Uses the existing professional evaluator shape from recompute_matches(),
-- request_required_days() as the date SSOT, and the configured freshness authority.

CREATE TABLE IF NOT EXISTS public.availability_opportunity_state (
  request_id uuid NOT NULL REFERENCES public.requests(id) ON DELETE CASCADE,
  freelancer_id uuid NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  is_test boolean NOT NULL DEFAULT false,
  reason text NOT NULL DEFAULT 'zero_overlap',
...
GRANT ALL ON public.availability_opportunity_state TO service_role;
ALTER TABLE public.availability_opportunity_state ENABLE ROW LEVEL SECURITY;
CREATE INDEX IF NOT EXISTS availability_opportunity_state_env_idx
  ON public.availability_opportunity_state (is_test, notified_at);
...
BEGIN
  FOR _row IN
    ...
    ORDER BY p.request_id, p.freelancer_id
  LOOP
...
CREATE OR REPLACE FUNCTION public.emit_availability_opportunity_notifications_live()
RETURNS integer LANGUAGE sql SECURITY DEFINER SET search_path TO 'public'
AS $$ SELECT public.emit_availability_opportunity_notifications(false); $$;
REVOKE ALL ON FUNCTION public.emit_availability_opportunity_notifications_live() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.emit_availability_opportunity_notifications_live() TO service_role;

CREATE OR REPLACE FUNCTION public.run_availability_opportunity_test()
RETURNS integer LANGUAGE sql SECURITY DEFINER SET search_path TO 'public'
AS $$ SELECT public.emit_availability_opportunity_notifications(true); $$;
REVOKE ALL ON FUNCTION public.run_availability_opportunity_test() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.run_availability_opportunity_test() TO service_role;

DO $do$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron')
     AND NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'emit-availability-opportunity-notifications') THEN
    PERFORM cron.schedule(
      'emit-availability-opportunity-notifications',
      '45 * * * *',
      'SELECT public.emit_availability_opportunity_notifications_live();'
    );
  END IF;
END;
$do$;