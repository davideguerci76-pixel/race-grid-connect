-- STEP 4C completion: use the existing request_required_days() function as the source of truth in the matching and request-confirmation consumers.
-- This is a forward-only source refactor. No data rows are changed. engagements.covered_days remains authoritative after confirmation.
...
DECLARE
  ...
  _required_days date[];
BEGIN
  SELECT r.season_dates, r.start_date, r.end_date, r.is_active, r.team_id
    INTO _season_dates, _start_date, _end_date, _is_active, _team_id
  ...
  WHERE r.id = _request_id
    AND (
      ...
      OR EXISTS (
        ...
        WHERE e.request_id = r.id AND e.freelancer_id = _uid
      )
    );
        ...
        ELSE languages_weight * (
          (SELECT COUNT(*)::numeric FROM jsonb_array_elements(r_langs) lreq
            WHERE EXISTS (
              ...
              WHERE lower(coalesce(flang->>'code','')) = lower(coalesce(lreq->>'code',''))
                AND (CASE lower(coalesce(flang->>'level','')) WHEN 'basic' THEN 1 WHEN 'intermediate' THEN 2 WHEN 'advanced' THEN 3 WHEN 'fluent' THEN 4 WHEN 'native' THEN 5 ELSE 0 END)
                    >= (CASE lower(coalesce(flang->>'level','')) WHEN 'basic' THEN 1 WHEN 'intermediate' THEN 2 WHEN 'advanced' THEN 3 WHEN 'fluent' THEN 4 WHEN 'native' THEN 5 ELSE 1 END)
            )
...
BEGIN
  ...
  -- SSOT for REQUEST REQUIRED DAYS. The freelancer-specific covered_days
  -- snapshot is still calculated below from real availability.
