-- UAT-LAUNCH-02 — Pit Call launch gate: LIVE governed by ACP toggle, TEST/DEMO always operational.
-- CAN_CREATE_PITCALL = env_is_test() OR flag_pitcall_creation_disabled < 1
-- env_is_test() is the existing server-side environment authority (profiles.is_test / admin_env_state);
-- nothing client-supplied is consulted.
CREATE OR REPLACE FUNCTION public.pitcall_creation_allowed()
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT public.env_is_test()
      OR public.get_setting_num('flag_pitcall_creation_disabled', 0) < 1;
$$;
REVOKE ALL ON FUNCTION public.pitcall_creation_allowed() FROM public, anon;
GRANT EXECUTE ON FUNCTION public.pitcall_creation_allowed() TO authenticated, service_role;

-- Enforce the gate inside the creation authority itself (previously only the
-- TanStack wrapper checked the flag, so a direct RPC call could bypass it).
